...
const labFallback = [
  { slug: 'sakuga-spark', name: 'Sakuga Spark', description: 'High-energy keyframe studies and impact timing.', accent: '#f2674a', glyph: '✦' },
  { slug: 'panel-punch', name: 'Panel Punch', description: 'Comic rhythm, bold cuts, and graphic transitions.', accent: '#f4b942', glyph: '▦' },
  { slug: 'frame-mold', name: 'FrameMold', description: 'Shape language experiments built one frame at a time.', accent: '#8bd5ca', glyph: '◈' },
  { slug: 'pixelpulse', name: 'PixelPulse', description: 'Tiny pixels, huge beats, and crisp loop logic.', accent: '#74a9ff', glyph: '▥' },
  { slug: 'inkdrift', name: 'InkDrift', description: 'Organic linework that moves like it is breathing.', accent: '#c69cff', glyph: '〰' },
  { slug: 'motion-rift', name: 'Motion Rift', description: 'Stretch, smear, and bend the space between poses.', accent: '#ff7eb6', glyph: '◒' }
];
...
function App() {
  ...
  const createProject = async ({ name, categorySlug }) => {
    ...
    setProjects((items) => [result.project, ...items]); setShowCreate(false); chooseProject(result.project);
  };
  ...
  return <div className="app-shell">
    ...
    <main className="content"><section className="intro"><div><p className="eyebrow">OPEN WORKSPACE / 2026</p><h1>动画实验，<span>从一帧开始。</span></h1><p className="lede">六个专注的实验室入口。把灵感变成可以反复推敲的运动。</p></div><button className="button primary" onClick={() => setShowCreate(true)}><Plus size={17} />新建项目</button></section>
      <section className="lab-grid">{labs.map((lab, index) => <article className="lab-card" key={lab.slug} style={{ '--accent': lab.accent }}><div className="lab-number">0{index + 1}</div><div className="lab-glyph">{lab.glyph}</div><div className="lab-copy"><h2>{lab.name}</h2><p>{lab.description}</p></div><button className="card-arrow" title={`从 ${lab.name} 创建项目`} onClick={() => { setShowCreate(true); setCreateLab(lab.slug); }}><ArrowUpRight size={18} /></button></article>)}</section>
      <section className="workspace-section"><div className="section-heading"><div><p className="eyebrow">YOUR PROJECTS / {projects.length.toString().padStart(2, '0')}</p><h2>最近的工作</h2></div><span className="section-meta"><Clock3 size={14} />自动保存已开启</span></div>{projects.length === 0 ? <div className="empty-projects"><div className="empty-icon"><Film size={22} /></div><div><h3>你的工作区还很安静</h3><p>选一个实验室，建立第一个可持续迭代的项目。</p></div><button className="button secondary" onClick={() => setShowCreate(true)}><Plus size={16} />开始创作</button></div> : <div className="project-list">{projects.map((project) => <button className={`project-row ${selected?.id === project.id ? 'active' : ''}`} key={project.id} onClick={() => chooseProject(project)}><span className="project-accent" style={{ background: project.accent }} /><span className="project-icon" style={{ color: project.accent }}>{project.glyph}</span><span className="project-details"><strong>{project.name}</strong><small>{project.categoryName} · {project.draftRevision ? `Draft ${String(project.draftRevision).padStart(2, '0')}` : 'Draft'}</small></span><span className="project-time">{formatDate(project.updatedAt)}</span><ArrowUpRight size={16} /></button>)}</div>}</section>
      {selected && <section className="workspace"><div className="workspace-head"><div><p className="eyebrow">WORKSPACE / {activeLab?.name?.toUpperCase()}</p><h2>{selected.name}</h2></div><SaveStatus state={saveState} /></div><div className="canvas-shell"><div className="canvas-toolbar"><span className="tool-label"><span className="tool-mark" style={{ background: activeLab?.accent }} />空白容器</span><span className="tool-note">可插拔工作区 · 即将支持画布、时间轴与节点</span><button className="icon-button" title="上传资源" onClick={() => setError('资源上传 API 已就绪，具体编辑器接入后可从这里导入。')}><Upload size={16} /></button></div><div className="canvas-placeholder"><div className="placeholder-cross"><span /><span /></div><div className="placeholder-copy"><h3>Workspace ready</h3><p>这是 {activeLab?.name} 的可插拔工作区。</p><small>先记录想法，下一步再把工具接进来。</small></div><textarea aria-label="实验笔记" value={draft.settings?.note || ''} onChange={(event) => updateDraft({ ...draft, settings: { ...draft.settings, note: event.target.value } })} placeholder="写下一句实验笔记…" /></div></div></section>}
